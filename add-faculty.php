...
if (isset($_POST['submitBtn'])) {
    ...
    // fetch id from faculty table where emailaddress is what we have entered..
    ...
    $sql = "SELECT id FROM faculty WHERE emailAddress = '$emailAddress'";
    if ($result = $mysqli->query($sql)) {
        ...
        if ($result->num_rows == 0) {
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $sql = "INSERT INTO login (user_id, email , password, role) VALUES ('$insertId', '$emailAddress', '$password', 'faculty')";
                if ($result = $mysqli->query($sql)) {
                    // check if number of affected rows is greater than 0
                    // that means insert query has been successfully executed.
                    if (mysqli_affected_rows($mysqli) > 0) {
                        $_SESSION['success-msg'] = "Faculty has been successfully created.";
                        echo '<script> location.replace("list-faculty.php"); </script>';
                    }
                }
            }
        } else {
...
<div class="page-heading">
    <div class="row">
        ...
        <div class="col-6 text-right">
            <a href="list-faculty.php" class="btn btn-primary">
                Faculty List
            </a>
        </div>
    </div>
...
<div class="add-new-user">
    <form action="" method="post">
        ...
        <div class="form-group row align-items-center">
            ...
            <div class="col-sm-10">
                <input type="text" required class="form-control" name="subject" id="subject" placeholder="Enter subject">
            </div>
        </div>
