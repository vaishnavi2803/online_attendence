...
if(isset($_GET['deleteUserId']) && $_GET['deleteUserId'] != ''){
    ...
    $sql = "DELETE FROM hod WHERE id = '$delete_userId'";
    $result= $mysqli->query($sql);

    $sql = "DELETE FROM login WHERE user_id = '$delete_userId' AND role = 'hod'";
    $result= $mysqli->query($sql);
}

if (isset($_POST['submitBtn'])) {
    $name = $_POST['name'];
    $sql = "SELECT * FROM hod WHERE fullName LIKE '%$name%' ORDER BY id DESC";
} else {
    $sql = "SELECT * FROM hod ORDER BY id DESC";
}
...
?>
        ...
        <div class="col-3 text-right">
            <a href="add-hod.php" class="btn btn-primary">
                Add New HOD
            </a>
        </div>
    </div>
                    ...
                    if (isset($rows) && count($rows) > 0) {
                        foreach ($rows as $key => $row) {
                            ...
                            echo "<tr>
                                ...
                                <td>
                                <a href='edit-hod.php?userId=" . $row['id'] . "' class='btn btn-outline-primary btn-sm'>Edit</a>
                                <a href='list-hod.php?deleteUserId=" . $row['id'] . "' class='btn btn-outline-danger btn-sm'>delet</a>
                                </td>
                            </tr>";
                        }
                    }
